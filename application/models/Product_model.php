...
class Product_model extends CI_Model
{
  ...
  {   
    $this->form_validation->set_rules('title', 'Category Title', 'required|is_unique[product_category.name]');
  ...
  {   
      ...
      $this->form_validation->set_rules('subCatName', 'Sub Category Title', 'required|is_unique[product_subcategory.name]');
      $name=$this->input->post("title");  
      if( $this->form_validation->run() == false ) 
      { 
          $this->session->set_flashdata('error', '<div class="alert alert-danger text-center">'.validation_errors().'</div>');
  ...
  public function allCategoriesWithProdNum(){
    ...
    public function allCategoriesWithSubCategories(){
        ...
        "select   
          ...
          `product_subcategory`.slug as 'subcat_slug',
          `product_category`.name as 'cat_name',
          `product_category`.id as 'cat_id',
          `product_category`.slug as 'cat_slug'
    ...
    public function subCategories($catSlug){
      $query=$this->db->query(
      "select   
        `product_subcategory`.name as 'name',
        `product_subcategory`.id as 'id' ,
        `product_subcategory`.slug as 'subCatSlug',
        `product_category`.slug as 'catSlug',
        `pictures`.pic_location  
      from product_subcategory  
        LEFT JOIN `product_category` ON
          product_category.id= product_subcategory.product_category_id
        LEFT JOIN `pictures` 
          ON product_subcategory.id=pictures.picture_of_id and pictures.picture_type='subcategory' 
          where  product_category.status <> 0 AND product_category.slug='".$catSlug."' AND product_subcategory.status <> 0 order by `product_category`.name "
      );
      return $query->result();    
  }

    public function allCategorieslimit($records){
      if($records=='active_records'){
        $query=$this->db->query(
          'select 
              ...
              LEFT JOIN `pictures` 
              ON product_category.id=pictures.picture_of_id where product_category.status=1 AND pictures.picture_type="category" limit 6'
        );
      }
  ...
  public function products($catSlug, $subCatSlug =""){

    $query=$this->db->query(
      "select products.* , product_category.name as 'cat_name' ,product_category.slug as 'catSlug', pictures.pic_location from products

        LEFT JOIN `product_subcategory` on `product_subcategory`.id=products.subcategory_id 
        LEFT JOIN `product_category` on `product_category`.id=product_subcategory.product_category_id 
        LEFT JOIN `pictures` on `pictures`.picture_of_id=products.id AND pictures.picture_type='Product' AND
        pictures.order_no=1

        where product_subcategory.slug='".$subCatSlug."' AND 
              products.status <> 0 AND
              product_category.status <> 0 AND
              product_subcategory.status <> 0    
         " );
  ...
  public function addProductsIntoDb(){
    ...
    if($this->input->post('prdSubmit')){
      ...
      $this->form_validation->set_rules('prdTitle', 'Title', 'required|is_unique[products.title]');
      ...
      $slug = url_title($this->input->post('prdTitle'), 'dash', true);
        ...
        if($result){
          ...
          $this->db->insert('seo_products',$array);
        }
...
// -------------------My Product Fuctions ------------------  // 
  ...
  public function update_product(){
      ...
      // echo $product_new_status =$this->input->post('product_new_status');
      ...
      $results['id']=$id;
      $results['action']=$action;
      $results['product_new_title']=$product_new_title;
      $results['product_new_brand']=$product_new_brand;
      $results['product_new_price']=$product_new_price;
      // $results['product_new_status']=$product_new_status;
  ...
  public function productinfo($slug){
    ...
    $query=$this->db->query(
      "select products.*,vendor_information.company_name,
              vendor_information.created_at,vendor_information.address,
              vendor_information.profile_picture,cities.city_name, 
              concat(users.first_name,' ',users.last_name) as 'user_name' ,
              users.contact_num,users.email,product_category.name as 'cat_name',
              product_subcategory.name as 'subcat_name' from products 
    ...
    $prdId=(int)$prdinfo[0]->id;
    $pquery=$this->db->query(" select pic_location, order_no from pictures where picture_of_id=$prdId and picture_type='Product' ");
